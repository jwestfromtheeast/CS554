...
export default function PokemonPage(props) {
  ...
  } else if (notFound || pokemonPageData.results.length === 0) {
    return <Page404 />;
  } else {
    if (parseInt(page) === 0) {
      return (
        <div>
          <Jumbotron className="bg">
            <h1 className="custom-header">Pokemon</h1>
            <ul className="custom-txt">
              {pokemonPageData.results.map((pokemon, index) => {
                return (
                  <li key={index}>
                    <Link to={`/pokemon/${parseId(pokemon.url)}`}>
            ...
            <Pagination>
              ...
              <LinkContainer to="/pokemon/page/1">
      ...
      return (
        <div>
          <Jumbotron className="bg">
            ...
            <Pagination>
              <LinkContainer to={`/pokemon/page/${parseInt(page) - 1}`}>
